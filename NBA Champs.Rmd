...
```
```{r}
 {kmeans(TotalScale, 5, nstart = 10 )$tot.withinss
}
...
TotalScale %>% 
ggplot() +
  geom_point(aes(x=ThreePointMakes, y=TotalTurnovers, color="red")) 
```
```{r}
teamcluster <- data.frame(TotalScale, k8$cluster)
```
```{r}
ggplot() +
  geom_point(data=teamcluster, aes(x=ThreePointMakes, y=TotalTurnovers, color=k8.cluster))
```
```{r}
